...
- **El motor de transcripción por defecto (whisper.cpp) empaqueta un binario de terceros sin firmar** en Windows y Linux: `whisper-cli` (más sus DLLs/.so) descargado en build desde los releases oficiales de `ggml-org/whisper.cpp` (v1.9.1), verificado por SHA-256 en `scripts/fetch-whisper-binaries.cjs`. Mantiene la misma postura de "sin firma de código" que el helper nativo de pegado (comparte el bloqueador de firma de arriba). En macOS no se empaqueta binario propio: la app recae automáticamente en el motor transformers.js. Si el sidecar no está presente o falla, el dispatcher usa transformers.js, por lo que la ausencia del binario nunca deja al usuario sin transcripción.
